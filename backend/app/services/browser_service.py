...
def click_apply_sync(
    ...
    if cookies:
        try:
            # Filter out Cloudflare bot-management cookies.
            # If we send a CF cookie generated by a Mobile IP from a Datacenter IP, 
            # Cloudflare instantly flags it as a hijacked session and blocks us.
            # Patchright will generate its own fresh CF clearance locally.
            safe_cookies = [
                c for c in cookies 
                if c.get("name") not in ["__cf_bm", "cf_clearance", "cf_ob_info", "cf_use_ob"]
            ]
